...
class Search:
    ...
    def is_cycle(self, node):
      visited = set()
      while (node.parent is not None):
        node_tuple = tuple(node.state.tiles)
        ...
        for depth in range(10000):
            ...
            while (len(frontier) > 0):
                ...
                if cur_node.depth < depth:
                  if self.is_cycle(cur_node) is False:
                    for child in self.get_children(cur_node):
                          frontier.append(child)
    ...
    def solve(self, input):
        ...
        path, expanded_nodes, time_taken, memory_consumed = self.run_dfs(root)
...
if __name__ == '__main__':
    agent = Search()
    initial_list = "1 0 2 4 5 7 3 8 9 6 11 12 13 10 14 15"
